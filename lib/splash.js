...
export default function splash ({ path, name, org, coc, logo, active, total, channels, large, iframe, gcaptcha_sitekey }){
  let div = dom('.splash',
    ...
    dom('p.status',
      active
        ? [
          dom('b.active', active), ' 人が接続中。',
          dom('b.total', total), ' 人が登録しています。'
        ]
        : [dom('b.total', total), ' 人が登録しています。']
    ),
    dom('form id=invite',
      ...
      dom('input.form-item type=email name=email placeholder=メールアドレスを入力する'
        + (!iframe ? 'autofocus' : '')),
      dom('br'),
      ...
      dom('button.loading', '招待メールを受け取る')
    ),
    dom('p.signin',
      'or ',
      dom(`a href=https://${org}.slack.com target=_top`, 'ログインはこちら'),
      '.'
    ),
    !iframe && dom('footer',
      'powered by ',
